...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <div class="bg-white border border-zinc-200 rounded-3xl p-6 sm:p-8 shadow-sm">
        <form action="/admin/makanan/store" method="POST" enctype="multipart/form-data" class="space-y-5">
            <?= csrf_field() ?>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="text" 
                               id="input_harga"
                               name="harga" 
                               value="<?= old('harga') ? number_format((int)preg_replace('/[^0-9]/', '', old('harga')), 0, ',', '.') : '' ?>" 
                               required 
                               placeholder="45.000"
                ...
                <div>
                    ...
                    <input type="number" 
                           ...
                           required 
                           min="0"
                           class="w-full bg-zinc-50 border <?= isset($errors['stok']) ? 'border-red-500 focus:ring-red-500' : 'border-zinc-200 focus:ring-primary' ?> rounded-xl px-3.5 py-2.5 text-xs text-zinc-900 focus:outline-none focus:ring-2 focus:ring-primary focus:bg-white transition">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 items-start">
                <div class="sm:col-span-2 space-y-3">
                    ...
                    <label for="foto_input" id="drop_zone" class="block border-2 border-dashed <?= isset($errors['foto']) ? 'border-red-400 bg-red-50/20' : 'border-zinc-200 hover:border-primary/50 bg-zinc-50/50' ?> rounded-2xl p-4 text-center cursor-pointer transition relative group">
                        ...
                        <div id="upload_prompt" class="space-y-2 py-2 pointer-events-none">
                            <div class="w-10 h-10 rounded-full bg-zinc-100 flex items-center justify-center mx-auto text-zinc-400 group-hover:text-primary group-hover:bg-primary/10 transition">
                                <i data-lucide="image-plus" class="w-5 h-5"></i>
                            </div>
                            <div class="text-xs text-zinc-600">
                                <span class="font-bold text-primary">Pilih Berkas Foto</span> dari komputer
                            </div>
                            <p class="text-[10px] text-zinc-400">PNG, JPG, JPEG, WEBP (Maksimal 2 MB)</p>
                        </div>

                        <!-- Preview Area -->
                        <div id="preview_container" class="hidden flex items-center gap-4 text-left pointer-events-none">
                            <img id="image_preview" src="" alt="Preview" class="w-16 h-16 object-cover rounded-xl border border-zinc-200 shadow-sm shrink-0">
                            <div class="overflow-hidden flex-1">
                                <p id="file_name" class="text-xs font-bold text-zinc-800 truncate"></p>
                                <p id="file_size" class="text-[10px] text-zinc-400 mt-0.5"></p>
                                <span class="text-[11px] font-semibold text-primary inline-block mt-1">Klik untuk mengganti berkas</span>
                            </div>
                        </div>
                    </label>
                    ...
                    <div class="pt-1">
                        <label class="block text-[10px] font-bold uppercase text-zinc-400 mb-1">Atau Gunakan Link URL Gambar (Opsional):</label>
                        <input type="url" 
                               name="gambar_url" 
                ...
                <div>
                    <label class="block text-xs font-bold uppercase tracking-wider text-zinc-600 mb-1.5">Rating Awal (0 - 5.0)</label>
                    <div class="relative">
                        <input type="number" 
                               step="0.1" 
                               min="1" 
...
<script>
    ...
    // Penanganan interaktif File Picker dan Preview Gambar
